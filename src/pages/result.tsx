...
const Result = () => {
   const{userAnswers, setUserAnswers, quizInfo, questions, setQuestions } = useContext(AppContext)
   ...
   const renderPercent  = () => {
    ...
    return (
      <p className="badge bg-primary">{percent.toFixed(2)} %</p>
    )
            ...
            <tbody>
              {
                questions.map((question) => {
                  const answers = userAnswers.find((answer) => answer.question === question.question)
                  ...
                  return (
                    <tr>
                      ...
                      <td className={hasAnswer ?(correct ? "text-success" : "text-danger") : "text-light" }>
