...
const QuestionCard= ({question, answers, callback, userAnswer, questionNumber, totalQuestion, onNext, onPrevious }: QuestionDetails) => {
  
  const [time, setTime] = useState<number>(totalQuestion*10)
  const{userAnswers, questions} = useContext(AppContext)
  const progressPercent =(questionNumber/totalQuestion)*100 

  LoadTimeHook({time, setTime})
  
  const handleDisabled = () => {
    const showedQuestion = questions.find((question) => 
    question.answers === answers)

    const selectedAnswer = userAnswers.find((userAnswer) => 
    userAnswer.question === showedQuestion?.question)

    return (
      selectedAnswer !== undefined || time <= 0
    )
  }

   return ( 
        <div className='position-relative mx-2 rounded rounded-4 bg-light bg-opacity-50'>
         <h4 className={time <=15 ? "text-danger" : "text-dark"}>Time:{time}s </h4>
        ...
        <div>
         <button
           ...
           <h4 className='text-danger'> <ArrowLeft/> </h4>  
         </button>
         <button
           onClick={onNext}
           ...
           <h4 className='text-danger'> <ArrowRight/> </h4>  
          </button>
         </div>
